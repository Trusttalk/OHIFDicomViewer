...
new CopyWebpackPlugin({
      patterns: [
        ...
        // We set context to the app root (path.resolve('./')) to ensure we find 'public', not 'src/public'
        {
          from: 'public',
          to: DIST_DIR,
          context: path.resolve(__dirname, '../'),
          globOptions: {
            // Ignore config files (we handle them below) and templates
            ignore: ['**/config/**', '**/html-templates/**'],
          },
        },
        // 2. Explicitly copy the Config File
        {
          // We build the absolute path to ensure Windows finds it
          from: path.resolve(__dirname, '../public', process.env.APP_CONFIG || 'config/default.js'),
          to: path.join(DIST_DIR, 'app-config.js'),
          noErrorOnMissing: false,
        },
        // 3. Copy Microscopy Assets (The Fix for the "unable to locate" error)
        {
          // dynamically find where dicom-microscopy-viewer is installed
          from: path.join(
            path.dirname(require.resolve('dicom-microscopy-viewer/package.json')),
            'dist/dynamic-import'
          ),
          to: DIST_DIR,
          toType: 'dir',
